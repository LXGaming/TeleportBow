...
package io.github.lxgaming.teleportbow.managers;

import io.github.lxgaming.teleportbow.TeleportBow;
import io.github.lxgaming.teleportbow.commands.AbstractCommand;
...
public final class CommandManager {
    ...
    public static CommandResult process(AbstractCommand abstractCommand, CommandSource commandSource, String message) {
        ...
        TeleportBow.getInstance().getLogger().debug("Processing {} for {}", command.get().getPrimaryAlias().orElse("Unknown"), commandSource.getName());
    ...
    public static boolean registerCommand(Class<? extends AbstractCommand> commandClass) {
        if (getCommandClasses().contains(commandClass)) {
            TeleportBow.getInstance().getLogger().warn("{} has already been registered", commandClass.getSimpleName());
            return false;
        }
        ...
        Sponge.getCommandManager().register(TeleportBow.getInstance().getPluginContainer(), command.get(), command.get().getAliases().toArray(new String[0]));
    ...
    public static boolean registerAlias(AbstractCommand command, String alias) {
        if (Toolbox.containsIgnoreCase(command.getAliases(), alias)) {
            TeleportBow.getInstance().getLogger().warn("{} has already been registered for {}", alias, command.getClass().getSimpleName());
            return false;
        }
        
        command.getAliases().add(alias);
        TeleportBow.getInstance().getLogger().debug("{} registered for {}", alias, command.getClass().getSimpleName());
        return true;
    }
    ...
    public static boolean registerCommand(AbstractCommand parentCommand, Class<? extends AbstractCommand> commandClass) {
        if (parentCommand.getClass() == commandClass) {
            TeleportBow.getInstance().getLogger().warn("{} attempted to register itself", parentCommand.getClass().getSimpleName());
            return false;
        }
        ...
        TeleportBow.getInstance().getLogger().debug("{} registered for {}", commandClass.getSimpleName(), parentCommand.getClass().getSimpleName());
        return true;
    }
